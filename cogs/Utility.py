...
class Utility:
    ...
    async def mute(self, ctx, members: commands.Greedy[discord.Member],
                   minutes: typing.Optional[int] = 10, *, reason='no reason'):
        '''Apply muted role'''
        ...
        if len(members) == 1:
            ...
            await ctx.send((f'{", ".join(member.mention for member in members)} were '
                            'muted for {minutes} minute(s) because {reason}.'))
            await channel.send((f'{", ".join(member.mention for member in members)} were muted for {minutes}'
                                ' minute(s) because {reason}.'))
    ...
    async def kick(self, ctx, members: commands.Greedy[discord.Member], *, reason='no reason'):
        ...
        if len(members) == 1:
            ...
            kick_msg = f'{", ".join({member.display_name} for member in members)} have been kicked because {reason}.'
            await ctx.send(kick_msg)
    ...
    async def ban(self, ctx, members: commands.Greedy[discord.Member], *, reason='no reason'):
        ...
        if len(members) == 1:
            ...
            ban_msg = f'{", ".join({member.display_name} for member in members)} have been banned because {reason}.'
